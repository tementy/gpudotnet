...
namespace OpenCL.Net
{
    ...
    {
        public const string Library = "OpenCL.so";
        ...
        private static extern IntPtr clCreateProgramWithBinary(IntPtr context,
                                                               ...
                                                               [In] [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1, ArraySubType = UnmanagedType.SysUInt)] ErrorCode[] binariesStatus,
                                                               out ErrorCode errcodeRet);
        public static Program CreateProgramWithBinary(Context context,
                                                         ...
                                                         ErrorCode[] binariesStatus,
                                                         out ErrorCode errcodeRet)
        {
            using (var binariesPtr = binaries.Array.Pin())
                return new Program(clCreateProgramWithBinary((context as IHandle).Handle, numDevices, deviceList, lengths, binariesPtr, binariesStatus, out errcodeRet));
        }
        ...
        public static ErrorCode SetKernelArg(Kernel kernel, uint argIndex, long argSize, object argValue)
        {
            using (var argPtr = argValue.Pin())
                return clSetKernelArg((kernel as IHandle).Handle, argIndex, (IntPtr)argSize, argPtr);
        }
